...
Description: Various heuristic functions that returns the heuristic distance from one state to the goal state.
"""
...
class Heuristic:
    ...
    def manhattan(curState, problem):
        ...
        :return: The Manhattan distance between the current's state position & goal position
        """

        # Manhattan distance is the sum of the vertical and horizontal distances between two points.
    ...
    def euclidean(curState, problem):
        ...
        :param curState: The current search state
        :param problem: The A* search problem instance for this maze
        :return: The Euclidean distance between the current's state position & goal position
        """

        # Euclidean distance is the distance of the straight line that connects two points.
        pos1 = curState.getPos()
        pos2 = problem.getGoalPosition()
        dx, dy = Heuristic.finddXdY(pos1, pos2)
    ...
    def diagonal(curState, problem):
        ...
        :param curState: The current search state
        :param problem: The A* search problem instance for this maze
        :return: The Diagonal distance between the current's state position & goal position
        """
        pos1 = curState.getPos()
        pos2 = problem.getGoalPosition()
        dx, dy = Heuristic.finddXdY(pos1, pos2)
    ...
    def fancy_manhattan(curState, problem):
        ...
        :param problem: The A* search problem instance for this maze
        :return: The Manhattan distance between the current's state position & goal position
                  with additional award based on success rate of passing goal test
        """
    ...
    def forecast_manhattan(curState, problem):
        ...
        :param problem: The A* search problem instance for this maze
        :return: The Manhattan distance between the current's state position & goal position
                  with additional penalty based on success rate of neighbour's legality.
        """
        manhattanDist = Heuristic.manhattan(curState, problem)
        neighbours = problem.maze.getValidNeighbors(curState.getxCoordinate(), curState.getyCoordinate(), curState.getDice())
        validNeighbours = [neighbour for neighbour in neighbours if neighbour[2] != 6]
